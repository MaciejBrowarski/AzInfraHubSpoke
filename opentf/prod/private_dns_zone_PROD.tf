...
resource "azurerm_private_dns_zone" "PROD_private_dns_zone" {
...
resource "azurerm_private_dns_zone_virtual_network_link" "PROD_private_dns_zone_link" {
  depends_on = [
    azurerm_private_dns_zone.PROD_private_dns_zone
  ]
  ...
  #virtual_network_id    = azurerm_virtual_network.PROD_vn["${each.value.vnet_resource_group}.${each.value.vnet_name}"].id
